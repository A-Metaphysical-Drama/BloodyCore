...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    if (target)
    {
        ...
        */
        ...
        if (m_spellInfo->AttributesEx2 == SPELL_ATTR2_UNK20 && m_spellInfo->AttributesEx & SPELL_ATTR1_UNK9 && target->HasInArc(static_cast<float>(M_PI), m_caster->GetPositionX(), m_caster->GetPositionY())
        ...
        if ((m_spellInfo->Attributes == (SPELL_ATTR0_UNK4 | SPELL_ATTR0_NOT_SHAPESHIFT | SPELL_ATTR0_UNK18 | SPELL_ATTR0_STOP_ATTACK_TARGET)) && !target->HasInArc(static_cast<float>(M_PI), m_caster->GetPositionX(), m_caster->GetPositionY()))
        {
            SendInterrupted(2);
